...
- **±5분** (`E` 는 +5분만) — 데드라인 기반이라 `endsAt += 300_000` 이면 끝. `plannedMs` 도
  함께 늘려 기록이 "25분 계획했는데 30분 했다" 가 아니라 정직하게 남는다. 줄이는 쪽은
  `EXTEND_FLOOR_MS`(10초) 아래로 못 내려간다 — 남은 시간을 음수로 만들면 절전 복구
  로직이 "오래 잠들었다 깨어남"으로 오판한다.
...
### 5.1 ★ 작업 끝에 preview 서버를 내리지 않는다 — 재시작이 기본이다

수정·검증이 끝났다고 `preview_stop` 으로 서버를 내린 채 끝내지 않는다. 사용자가 바로
이어서 브라우저로 써 볼 수 있어야 하므로, **떠 있던 서버는 최신 코드로 재시작해서
띄워 둔 채로 마친다.**

- 프론트(`ui/**`)만 고쳤다면 정적 파일이라 서버를 안 내려도 새로고침으로 반영된다 —
  이 경우는 그냥 살려 두면 된다.
- `server/**`(Python) 를 고쳤다면 실행 중인 프로세스가 옛 코드를 물고 있으므로
  `preview_stop` 뒤 `preview_start` 로 **재시작**해야 반영된다. (서버 재사용 시
  `reused: true` 로 조용히 예전 프로세스를 다시 쓰는 경우가 있으니 방금 리스타트했는지
  결과를 확인한다.)
- 검증용으로 설정·타이머 상태를 임시로 바꿨다면(예: 초 단위 빠른 사이클 테스트) 되돌리는
  것과 별개로, **서버 자체는 끄지 않는다.**
